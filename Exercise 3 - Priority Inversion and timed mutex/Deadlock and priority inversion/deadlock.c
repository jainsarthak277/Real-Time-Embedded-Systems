...
void *grabRsrcs(void *threadid)
{
   ...
   if((int)threadid == THREAD_1)
   {
     ...
     rsrcBCnt++;
     printf("THREAD 1 got A and B on attempt %d\n", rsrcACnt);
   ...
   else
   {
	while(!key)
	{
     		...
     		if(pthread_mutex_trylock(&rsrcA) == EBUSY)
    		{
			pthread_mutex_unlock(&rsrcB);
			printf("Task 2 deadlocked, resetting mutex locks\n");
    		}
     		else 
    		{
     			rsrcACnt++;
     			printf("THREAD 2 got B and A on attempt %d\n", rsrcBCnt);
     			pthread_mutex_unlock(&rsrcA);
     			pthread_mutex_unlock(&rsrcB);
